...
- `ExtraFields`, serialized as additional XML blocks for fields not yet typed as first-class value objects.
...
The default `XmllintXsdValidator` validates against `EnvelopedSignature.xsd`. Inject another `XsdValidator` when the application needs a different validation runner.
